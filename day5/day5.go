...
func (s *Stack) Pop(amount int) (r []rune) {
	r = s.items[len(s.items)-amount : len(s.items)]
	...
	for _, item := range itemsArr {
		...
		for i := 1; i <= len(item)-2; i += 4 {
			...
			if item[i] != ' ' {
				...
				stacks[iter].addToBottom(rune(item[i]))
			}
			if iter == 8 {
				iter = -1
			}
...
func Part2(input string) string {
	...
	for _, line := range lines {
		...
		from, _ := strconv.Atoi(nums[2])
		to, _ := strconv.Atoi(nums[3])
		moving := stacks[from-1].Pop(amount)
		stacks[to-1].Push(moving)
	}
	var res string
	for _, stack := range stacks {
		res += fmt.Sprintf("%c", stack.Pop(1))

	}
	res = regexp.MustCompile(`[^A-Z0-9]+`).ReplaceAllString(res, "")
	return res
}
